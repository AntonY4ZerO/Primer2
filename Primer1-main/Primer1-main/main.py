...
k, T, C, L = symbols('k T C L')
#1 способ
C_ost = 200000 #Изменено правильно
Am_lst = []
C_ost_lst = []
for i in range(16): #Изменено правильно
  Am = (C-L)/T
  C_ost -=Am.subs({C:200000, T:16, L:0}) #Изменено правильно
  Am_lst.append(round(Am.subs({C:200000, T:16, L:0}),2)) #Изменено правильно
  C_ost_lst.append(round(C_ost,2))
print("Am_lst:" , Am_lst)
print("C_ost_lst:" , C_ost_lst)
print('\n-------------------------------------------------------------\n')
...
for i in range(16):
    Am = k * 1/T * (C - Aj)
    C_ost -= Am.subs({C: 200000, T: 16, k: 2})
    Am_lst_2.append(round(Am.subs({C: 200000, T: 16, k: 2}), 2))
    Aj += Am
    C_ost_lst_2.append(round(C_ost, 2))
